...
function validAppearance(value: unknown): Appearance {
  ...
  if (!value || typeof value !== 'object') {
    throw new Error('Invalid subtitle appearance');
  }

  const data = value as Record<string, unknown>;

  // 2. Read bounded sizes and six-digit colors.
  ...
  // 3. Return only supported appearance settings.
  return {
    sourceSize: number('sourceSize', 16, 56),
    sourceColor: color('sourceColor'),
    sourceBottom: number('sourceBottom', 4, 35),
    translationSize: number('translationSize', 16, 56),
...
function validSettings(value: unknown): Settings {
  // 1. Validate and normalize the submitted provider and language settings.
  if (!value || typeof value !== 'object') {
  ...
  if (
    ...
    !explanationLanguage ||
    explanationLanguage.length > 64
  ) {
    throw new Error('Enter source and explanation languages');
  }

  if (typeof data.includeSecondary !== 'boolean' || typeof data.noKeyRequired !== 'boolean') {
    throw new Error('Invalid settings');
  }
...
export class Session {
    ...
    this.settings =
      stored && typeof stored === 'object'
        ? {
            ...
            appearance: {
              ...DEFAULT_APPEARANCE,
              ...(stored as { appearance?: object }).appearance
            }
          }
        : { ...DEFAULTS };
  ...
  start(): void {
    ...
    const on = (name: string, callback: () => void) =>
      this.listeners.push({ name, id: event.on(name, callback) });
    ...
    // 2. Initialize immediately when installation happens in an open player.
    // Installing into an already open player does not replay iina.window-loaded.
    if (this.host.raw.core.window.loaded === true) {
      this.windowLoaded();
    }
  }

  private windowLoaded(): void {
    // 1. Initialize each live player window only once.
    if (this.windowInitialized || this.closed) {
      return;
    }

    this.windowInitialized = true;

    // 2. Wait for the native overlay before loading its page.
    ...
    this.listeners.push({
      name,
      id: event.on(name, () => {
        if (this.overlayPageRequested || this.closed) {
          return;
        }
    ...
    // 3. Synchronize playback and start the session timer.
    this.mediaChanged();
    this.timer = setInterval(() => this.tick(), 100);
  }

  private loadViews(): void {
    // 1. Load the overlay before attaching its message handlers.
    ...
    // 2. Connect sidebar actions to the owning player session.
    ...
    // 3. Track settings drafts and native sidebar visibility.
    sidebar.onMessage('settingsView', (value) => {
      ...
      if (!this.settingsOpen || data?.view === 'ai') {
    ...
    sidebar.onMessage('visibility', (value) => {
      ...
      if (hidden === true && this.host.windowVisible) {
        ...
        if (this.conversation && !this.settingsOpen) {
  ...
  private refreshSubtitleMenu(): void {
    ...
    if (signature === this.subtitleMenuState) {
      return;
    }
    ...
    menu.removeAllItems();
    menu.addItem(
      menu.item('Toggle Neden Panel', () => this.togglePanel(), { keyBinding: 'Alt+Meta+g' })
    );
    menu.addItem(
      menu.item(
        'Add SRT/VTT File…',
    ...
    for (const [name, selectedId, select] of [
      ...
      group.addSubMenuItem(
        menu.item('Off', () => select(0), { selected: selectedId === null || selectedId === 0 })
      );
      for (const track of tracks) {
        const title = track.formattedTitle || track.title || `Track ${track.id}`;
        group.addSubMenuItem(
          menu.item(title, () => select(track.id), { selected: selectedId === track.id })
        );
      }
      menu.addItem(group);
    }
  ...
  private selectSubtitleTrack(value: unknown): void {
    // 1. Reject invalid roles, missing tracks, and selections from old media.
    if (!value || typeof value !== 'object') {
      return;
    }

    const { role, id, epoch } = value as {
      role?: unknown;
      id?: unknown;
      epoch?: unknown;
    };
    ...
    if (id !== 0 && !this.host.subtitleTracks.some((track) => track.id === id)) {
      return;
    }

    // 2. Apply the track and refresh the overlay and sidebar.
    if (role === 'source') {
      this.host.selectSource(id as number);
    } else {
  ...
  private mediaChanged(): void {
    // 1. Cancel work owned by the previous media.
    if (this.closed) {
      return;
    }
    ...
    // 3. Clear the old views and synchronize the new tracks.
    this.host.setOverlayClickable(false);
    if (this.overlayReady) {
      this.host.toOverlay('clear', {});
    }
    if (this.overlayReady) {
      this.host.toOverlay('translation', '');
    }
    this.host.hideSidebar();
    this.sidebarVisible = false;
    this.syncTracks();
  ...
  private syncTracks(): void {
    ...
    if (sourceId !== this.sourceId) {
      ...
      if (track && sourceId !== null) {
        try {
          ...
          this.status = error instanceof Error ? error.message : 'Subtitle file is unreadable';
        }
      } else {
        this.status = 'Select an external UTF-8 SRT or VTT source subtitle track.';
      }
    }
    ...
    if (secondaryId !== this.secondaryId) {
      ...
      if (this.host.externalTrack(secondaryId) && secondaryId !== null) {
        try {
          this.secondary = this.readCues(secondaryId);
        } catch {
          /* display uses IINA's current text; context has no parsed secondary cues */
        }
      }
    }
    if (changed) {
  ...
  private updateCue(force = false): void {
    ...
    // 2. Synchronize native visibility and secondary subtitle text.
    const showSecondary = sourceReady && this.secondaryId !== null && this.secondaryId !== 0;
    if (showSecondary) {
      this.host.ownSecondary();
    } else {
      this.host.restoreSecondary();
    }
  ...
  private seek(): void {
    // 1. Distinguish the replay seek from a user-initiated seek.
    const replay = this.replay;
    if (replay) {
      const position = this.host.positionMs;
      const initialSeek =
        replay.awaitingInitialSeek &&
        Date.now() - replay.startedAt < 1_500 &&
        (Math.abs(position - replay.startMs) <= 300 ||
          Math.abs(position - replay.returnPositionMs) <= 300);
  ...
  private selectionReceived(value: unknown): void {
    ...
    if (
      this.sidebarVisible &&
      this.lastSelection?.key === key &&
      Date.now() - this.lastSelection.at < 500
    ) {
      return;
    }
  ...
  private explain(value: unknown): void {
    // 1. Verify that the submitted selection still belongs to this session.
    if (
      !this.overlayEnabled ||
      !this.source ||
      !this.pending ||
    ...
    if (
      submitted.text !== this.pending.text ||
      submitted.start !== this.pending.start ||
      submitted.end !== this.pending.end ||
      submitted.cue?.index !== this.pending.cue.index ||
      submitted.cue?.trackId !== this.sourceId
    ) {
      return;
    }

    // 2. Capture the selected cue and neighboring context.
    ...
    try {
      const context = contextForCue(
        this.source,
        sourceCue.index,
        this.secondary,
        this.settings.includeSecondary,
        {
          sourceDelayMs: this.host.delayMs,
          secondaryDelayMs: this.host.secondaryDelayMs,
          speed: this.host.subtitleSpeed
        }
      );
      const snapshot: ContextSnapshot = {
        selection: {
          mediaEpoch: this.mediaEpoch,
          sourceTrackId: this.sourceId!,
          cueIndex: sourceCue.index,
          cueStartMs: sourceCue.startMs,
          cueEndMs: sourceCue.endMs,
          cueText: sourceCue.text,
          rangeStartUtf16: pending.start,
          rangeEndUtf16: pending.end,
          exactText: pending.text
        },
        before: context.before.map((cue) => ({ ...cue })),
        after: context.after.map((cue) => ({ ...cue })),
        secondary: context.secondary.map((cue) => ({ ...cue })),
        sourceLanguage: this.settings.sourceLanguage,
        explanationLanguage: this.settings.explanationLanguage
      };

      // 3. Replace the conversation, pause playback, and start the explanation.
      this.cancelRequest();
      ...
      if (this.sidebarReady) {
        this.host.toSidebar('showConversation', {});
      }
      this.host.showSidebar();
      this.sidebarVisible = true;
  ...
  private launch(request: { owner: RequestOwner; messages: unknown }): void {
    // 1. Validate the configured endpoint and read its key in privileged code.
    ...
    try {
      ...
      if (!this.settings.model) {
        throw new Error('Set an API model in Settings');
      }

      const key = this.settings.noKeyRequired
        ? null
        : this.credentials.readForRequest(endpoint.base);
      if (!this.settings.noKeyRequired && !key) {
        throw new Error('Save an API key for this endpoint in Settings');
      }

      // 2. Build the provider request without exposing the key to a view.
      const deepSeekFlash =
        /^https:\/\/api\.deepseek\.com(?:\/|$)/.test(endpoint.base) &&
      ...
      // 3. Start the helper with explicit request ownership.
      const stream = new NativeStream(
        this.host.raw,
        this.host.helperPath(),
        this.host.requestDirectory(++this.nextStreamId),
        payload,
        (record) => this.streamRecord(request.owner, record)
      );
  ...
  private streamRecord(owner: RequestOwner, record: StreamRecord): void {
    if (
      !this.active ||
      !this.sameOwner(this.active.owner, owner) ||
      !this.conversation?.accepts(owner)
    ) {
      return;
    }

    if (record.kind === 'delta') {
      this.conversation.delta(owner, record.value);
    } else if (record.kind === 'done') {
      this.conversation.complete(owner);
      this.status = 'Complete';
    } else if (record.kind === 'error') {
      this.conversation.fail(owner, record.value);
      this.status = `Request failed: ${record.value}`;
    } else if (record.kind === 'cancelled') {
      this.conversation.fail(owner, 'Cancelled');
      this.status = 'Cancelled';
    }
    this.render();
  }

  private sameOwner(current: RequestOwner, incoming: RequestOwner): boolean {
    return (
  ...
  private followUp(value: unknown): void {
    if (!this.conversation || !value || typeof value !== 'object') {
      return;
    }
    ...
    try {
      ...
      this.status = error instanceof Error ? error.message : 'Could not send follow-up';
      this.render();
    }
  }
  ...
  private retry(): void {
    ...
    try {
      ...
      this.status = error instanceof Error ? error.message : 'Retry unavailable';
      this.render();
    }
  }

  private pauseForConversation(): void {
    if (this.resume || !this.host.playable || this.mediaUrl !== this.host.mediaUrl) {
      return;
    }
  ...
  private hideConversation(): void {
    // 1. Stop active work while retaining the conversation.
    if (!this.conversation) {
      return;
    }
    ...
    // 2. Resume only the media that this conversation paused.
    const resume = this.resume;
    this.resume = null;
    if (
      resume &&
      resume.epoch === this.mediaEpoch &&
      resume.url === this.host.mediaUrl &&
      this.host.playable
    ) {
      this.host.resume();
    }
  ...
  private disableOverlay(): void {
    // 1. Stop conversation work and discard transient view state.
    this.finishReplay(true);
    this.discardAppearancePreview();
  ...
  private swapSubtitleOrder(): void {
    ...
    try {
      ...
      this.settings = next;
      if (this.overlayReady) {
        this.host.toOverlay('subtitleOrder', next.secondaryBelowSource);
      }
      this.render();
    } catch (error) {
  ...
  private saveAppearance(value: unknown): void {
    try {
      // 1. Validate and persist the appearance before replacing saved state.
      const appearance = validAppearance(value);
      const next = { ...this.settings, appearance };
      this.host.raw.preferences.set('settings', next);
      this.host.raw.preferences.sync();

      // 2. Commit the preview to the overlay and refresh its cue.
      this.settings = next;
      ...
      this.status = error instanceof Error ? error.message : 'Could not save subtitle appearance';
    }
    this.render();
  }

  private previewAppearance(value: unknown): void {
    if (!this.settingsOpen || !this.overlayEnabled) {
      return;
    }
    ...
    if (this.overlayReady) {
      this.host.toOverlay('appearance', this.appearancePreview);
    }
    this.updateCue(true);
  }

  private discardAppearancePreview(): void {
    if (!this.appearancePreview) {
      return;
    }
  ...
  private toggleReplay(): void {
    ...
    if (
      !conversation ||
      conversation.context.selection.mediaEpoch !== this.mediaEpoch ||
    ...
    const endMs =
      duration !== null && Number.isFinite(duration) && duration > 0
        ? Math.min(duration * 1000, cueEndMs)
        : cueEndMs;
    if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || endMs - startMs < 100) {
      return;
    }
    ...
    this.replay = {
      ...
      startMs,
      endMs,
      returnPositionMs: this.host.positionMs,
      wasPaused: this.host.paused,
      startedAt: Date.now(),
      phase: 'starting',
      sawPlaying: false,
  ...
  private tickReplay(): void {
    // 1. Reject replay state that no longer belongs to the current media and track.
    const replay = this.replay;
    if (!replay) {
      return;
    }
    ...
    if (
      replay.epoch !== this.mediaEpoch ||
      replay.url !== this.host.mediaUrl ||
      this.conversation?.context.selection.sourceTrackId !== this.host.sourceId ||
      replay.conversationId !== this.conversation?.id
    ) {
      this.replay = null;
      this.render();
    ...
    // 2. Wait for playback to enter the cue, with a bounded startup window.
    const position = this.host.positionMs;
    if (replay.phase === 'starting') {
      if (position >= replay.startMs - 250 && position < replay.endMs) {
    ...
    // 3. Respect user interruptions and return after the cue finishes.
    if (!this.host.paused) {
      replay.sawPlaying = true;
    }
    if (
      (replay.sawPlaying && this.host.paused) ||
      position < replay.startMs - 500 ||
      position > replay.endMs + 1_000
    ) {
      this.replay = null;
      this.render();
  ...
  private render(): void {
    // 1. Resolve the public endpoint status without reading a saved key.
    if (!this.sidebarReady) {
      return;
    }
    ...
    try {
      ...
      /* an unsaved invalid endpoint has no credential status */
    }

    // 2. Publish player and conversation state with credential presence only.
    const conversation = this.conversation;
    this.host.toSidebar('state', {
      ...
      secondaryId: this.host.secondaryId,
      subtitleTracks: this.host.subtitleTracks.map((track) => ({
        id: track.id,
        title: track.formattedTitle || track.title || `Track ${track.id}`,
  ...
  private tick(): void {
    // 1. Synchronize media, replay, and subtitle state.
    if (this.closed) {
      return;
    }

    if (this.host.mediaUrl !== this.mediaUrl) {
      this.ended = false;
      this.mediaChanged();
    }
    this.tickReplay();
    ...
    // 2. Drain active and cancelled helpers on the plugin execution path.
    const active = this.active;
    if (active && active.stream.pump() && this.active === active) {
      this.active = null;
    }
  ...
  private teardown(): void {
    // 1. Invalidate requests and clear state owned by this window.
    this.closed = true;
    this.replay = null;
    this.conversation?.stop();
    this.cancelRequest();
    ...
    // 2. Restore native rendering and remove the timer and listeners.
    this.host.restorePrimary();
    this.host.restoreSecondary();
    this.host.hideOverlay();
    if (this.timer) {
...
if (typeof iina !== 'undefined') {
  new Session(new IinaHost(iina, PLUGIN_ID)).start();
}
