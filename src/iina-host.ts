...
export type RawIina = {
  ...
  overlay: {
    ...
    postMessage(name: string, value: string): void;
    show(): void;
    hide(): void;
...
export class IinaHost {
  ...
  get sourceId(): number | null {
    return this.raw.core.subtitle.id;
  }
  get secondaryId(): number | null {
    return this.raw.core.subtitle.secondID;
  }
  get positionMs(): number {
    return (this.raw.core.status.position || 0) * 1000;
  }
  get paused(): boolean {
    return this.raw.core.status.paused;
  }
  get delayMs(): number {
    return (this.raw.mpv.getNumber('sub-delay') || 0) * 1000;
  }
  get secondaryDelayMs(): number {
    return (this.raw.mpv.getNumber('secondary-sub-delay') || 0) * 1000;
  }
  get subtitleSpeed(): number {
    return this.raw.mpv.getNumber('sub-speed') || 1;
  }
  get displayedSource(): string {
    return this.raw.mpv.getString('sub-text') || '';
  }
  get displayedStartMs(): number {
    return this.raw.mpv.getNumber('sub-start') * 1000;
  }
  get secondaryText(): string {
    return this.raw.mpv.getString('secondary-sub-text') || '';
  }
  get subtitleTracks(): SubtitleTrack[] {
    return this.raw.core.subtitle.tracks;
  }
  get windowVisible(): boolean {
    return this.raw.core.window.visible;
  }
  ...
  externalTrack(id: number | null): SubtitleTrack | null {
    return id === null
      ? null
      : (this.raw.core.subtitle.tracks.find((track) => track.id === id && track.isExternal) ??
          null);
  }
  ...
  readTrack(id: number): string {
    ...
    if (typeof text !== 'string') {
      throw new Error('Selected external subtitle file is unreadable');
    }
  ...
  chooseSubtitleFile(): Promise<string> {
    return this.raw.utils.chooseFile('Add SRT or VTT subtitles', {
      allowedFileTypes: ['srt', 'vtt']
    });
  }

  loadSubtitleFile(path: string): void {
  ...
  restorePrimary(): void {
    ...
    if (this.raw.mpv.getFlag('sub-visibility') === false) {
      this.raw.mpv.set('sub-visibility', this.ownedPrimary);
    }
    this.ownedPrimary = null;
  }

  ownSecondary(): void {
    if (this.ownedSecondary !== null) {
      return;
    }
  ...
  resume(): void {
    this.raw.core.resume();
  }
  seekTo(positionMs: number): void {
    this.raw.core.seekTo(positionMs / 1000);
  }
  showOverlay(): void {
    this.setOverlayClickable(false);
    this.raw.overlay.show();
  }
  setOverlayClickable(clickable: boolean): void {
    if (this.overlayClickable === clickable) {
      return;
    }
  ...
  hideOverlay(): void {
    this.setOverlayClickable(false);
    this.raw.overlay.hide();
  }
  showSidebar(): void {
    this.raw.sidebar.show();
  }
  hideSidebar(): void {
    this.raw.sidebar.hide();
  }

  toOverlay(name: string, data: unknown): void {
    this.raw.overlay.postMessage(name, encodeURIComponent(JSON.stringify(data)));
  }
  toSidebar(name: string, data: unknown): void {
    this.raw.sidebar.postMessage(name, encodeURIComponent(JSON.stringify(data)));
  }
