...
export function canonicalEndpoint(value: string): {
  ...
  // 1. Validate the endpoint without browser-only URL APIs.
  const match = value.trim().match(/^(https?):\/\/([^/?#]+)(\/[A-Za-z0-9._~/-]*)?\/?$/i);
  if (!match || match[2].includes('@')) {
    throw new Error('Enter an HTTPS API base URL without credentials or a query');
  }

  // 2. Validate the host, port, and loopback-only HTTP exception.
  ...
  if (scheme === 'http' && !['localhost', '127.0.0.1', '[::1]'].includes(host)) {
    throw new Error('HTTP is allowed only for loopback testing');
  }
  ...
  // 3. Normalize the path and default port for endpoint-bound credentials.
...
export class Credentials {
  constructor(private readonly keychain: Keychain) {
    if (
      typeof keychain.keychainRead !== 'function' ||
      typeof keychain.keychainWrite !== 'function'
    ) {
      throw new Error('IINA Keychain API is unavailable');
    }
  }

  private service(endpoint: string): string {
    return `api:${canonicalEndpoint(endpoint).base}`;
  }
