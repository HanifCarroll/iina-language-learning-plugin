...
const first = { trackId: 1, index: 2, text: 'Ben öyle\nbir insan mıyım?' };
const second = { trackId: 1, index: 3, text: 'Next line' };
...
test('natural cue advance during drag keeps original cue and pending phrase', () => {
  ...
  expect(state.finishDrag(4, 19)).toEqual({ cue: first, start: 4, end: 19, text: first.text.slice(4, 19) });
