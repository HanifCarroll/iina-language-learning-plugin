...
for (const item of evalCases) {
  test(`${item.id} synthetic prompt uses the production conversation path`, () => {
    ...
    expect(
      context.selection.cueText.slice(
        context.selection.rangeStartUtf16,
        context.selection.rangeEndUtf16
      )
    ).toBe(item.selected);
...
test('pass/fail grader requires completed text, all case criteria, rubric threshold, and human rationale', () => {
  ...
  const passing: ReviewItem = {
    ...
    blockers: [],
    rationale: 'Question morphology and whole-cue meaning are explained.'
  };

  expect(grade(item, passing).verdict).toBe('PASS');
  expect(grade(item, { ...passing, answer: 'A fluent but unlabeled answer.' }).verdict).toBe(
    'FAIL'
  );
...
test('follow-up grading does not require repeated initial headings', () => {
  ...
  expect(
    grade(item, {
      status: 'complete',
      answer: 'Here bir marks an indefinite person, rather than the number one.',
      scores: [2, 2, 1],
      checks: Object.fromEntries(Object.keys(item.checks).map((key) => [key, true])),
      blockers: [],
      rationale: 'Direct answer tied to the original phrase.'
    }).verdict
  ).toBe('PASS');
});
