...
test('stdout hook queues only; timer opens FIFOs and cancellation crosses the control pipe', async () => {
  // 1. Record helper output callbacks and private-pipe writes.
  ...
  const host = {
    ...
    utils: {
      exec: (_path: string, _args: string[], _cwd: null, stdout: (chunk: string) => void) => {
        hook = stdout;
  ...
  // 2. Verify that only the timer opens pipes and delivers decoded content.
  ...
  // 3. Send cancellation and discard later content while accepting its terminal frame.
  stream.cancel();
