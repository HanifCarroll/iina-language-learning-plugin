...
test('full future timeline, timing, repeated text, and secondary overlap', () => {
  const source = parseSubtitles(
    '\uFEFF1\r\n00:00:00,000 --> 00:00:02,000\r\nBen öyle\r\nbir insan mıyım?\r\n\r\n2\r\n00:00:04,000 --> 00:00:06,000\r\nSame\r\n\r\n3\r\n00:00:08,000 --> 00:00:10,000\r\nSame\r\n\r\n4\r\n00:00:12,000 --> 00:00:14,000\r\nEnd\r\n',
    'srt'
  );
  const secondary = parseSubtitles(
  ...
  expect(contextForCue(source, 1, secondary).after.map((cue) => cue.text)).toEqual(['Same', 'End']);
  expect(contextForCue(source, 1, secondary).secondary.map((cue) => cue.text)).toEqual([
    'Am I that kind of person?'
  ]);
  expect(contextForCue(source, 1, secondary, false).secondary).toEqual([]);
  expect(
    contextForCue(source, 1, secondary, true, {
      sourceDelayMs: 0,
      secondaryDelayMs: 7_000,
      speed: 1
    }).secondary
  ).toEqual([]);
});
...
test('three cues on both sides are available before they play', () => {
  ...
  expect(contextForCue(source, 3).before.map((cue) => cue.index)).toEqual([0, 1, 2]);
  expect(contextForCue(source, 3).after.map((cue) => cue.index)).toEqual([4, 5, 6]);
