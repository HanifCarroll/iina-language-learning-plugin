...
test('endpoint path, TLS, loopback, and credential boundaries', () => {
  expect(canonicalEndpoint(' HTTPS://Example.COM:443/v1/ ').requestUrl).toBe('https://example.com/v1/chat/completions');
  expect(canonicalEndpoint('http://127.0.0.1:47891').requestUrl).toBe('http://127.0.0.1:47891/chat/completions');
  for (const bad of ['http://example.com/v1', 'https://user:pass@example.com', 'https://example.com/v1?x=1', 'https://example.com/../v1']) {
...
test('Keychain replacement and endpoint isolation, no plaintext fallback', () => {
  ...
  let fail = false;
  const keychain = {
    keychainWrite: (service: string, name: string, value: string) => { if (fail) return false; entries.set(`${service}:${name}`, value); return true; },
