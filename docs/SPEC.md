...
| Secondary subtitles | Turkish/source text is selectable. When a secondary track is selected and the source overlay is usable, its text always appears above the source, with separate display controls. Native secondary rendering is restored when the overlay is disabled or cannot safely own the source. |
...
- Hiding the panel retains its most recent conversation in memory for reopening in that player window, including incomplete output from a cancelled stream. A new completed selection, media change, window close, provider/language/context change, or turning off the selectable overlay clears it. There is no history list.
...
The plugin renders selectable **primary/source** text. Whenever a secondary track is selected and the source overlay is usable, it draws IINA's current **secondary/translation** text immediately above the source and suppresses duplicate native secondary rendering, including during gaps between source cues. The two lines have separate size and color controls; source distance from the bottom and secondary gap above source control their placement. The secondary context sent to AI remains an independent setting. When the source overlay is unavailable, native secondary rendering is restored.
...
| In-plugin selectable-overlay switch off / overlay failure | Cancel active work, invalidate late results, close the conversation under the same-media resume rules, and restore native primary and secondary visibility owned by this window. Test this during an active stream. Do not hijack another window's playback. |
...
| A25 | **Appearance preview:** editing size, color, or placement previews immediately without saving. Apply persists; leaving Subtitles and native sidebar dismissal restore saved values while keeping the selected secondary line above the source. Saving AI settings alone does not commit the preview. |
