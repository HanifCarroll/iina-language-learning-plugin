...
The plugin renders selectable **primary/source** text. Whenever a secondary track is selected and the source overlay is usable, it stacks IINA's current **secondary/translation** text with the source and suppresses duplicate native secondary rendering, including during gaps between source cues. Secondary starts above source. A Subtitles-view button swaps the order and persists it without changing track roles, selection behavior, or AI context. The two lines have separate size and color controls; stack distance from the bottom and gap between lines control their placement. The secondary context sent to AI remains an independent setting. When the source overlay is unavailable, native secondary rendering is restored.
...
| A25 | **Appearance preview and order:** editing size, color, or placement previews immediately without saving. Apply persists; leaving Subtitles and native sidebar dismissal restore saved values. Swap subtitle positions persists the stack order independently, including after saving AI settings, and keeps source text selectable. Saving AI settings alone does not commit the preview. |
